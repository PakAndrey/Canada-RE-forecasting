...
df_cov = pd.read_csv(f'{FORECAST_PATH}/df_cov_hpi_toronto.csv')
df_cov["Date"] = pd.to_datetime(df_cov["Date"])
df_cov.set_index("Date", inplace=True)

target = "MLS Condo Price Benchmark"
features = ["Mortgage rate 5y log",  "SNLR","Absorption Rate"]
...
'''
	...
	•	**Absorption Rate**: The rate at which available homes are sold in a given market over a specific period, expressed as a percentage. It helps gauge market demand and inventory turnover, with higher rates indicating strong buyer activity.
'''
