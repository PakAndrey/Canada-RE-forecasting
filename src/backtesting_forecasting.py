...
def hist_forecast(df, target, features, split_time, horizon, y_lags, model, retrain=True):
    ...
    for i in range(horizon):
        X = np.array(forecast[len(forecast)-y_lags:] + list(val_df[features[y_lags:]].iloc[i])).reshape(1, -1)
    ...
    return pd.Series(forecast[y_lags:], index=val_df[:horizon].index) 

def backtesting(df, target, features, test_split_time, horizon, y_lags, model, shift=1, retrain=True):
    """
    ...
    """
    pred_times = [
        t for i, t in enumerate(pd.date_range(start=test_split_time, periods=len(df[test_split_time:])-horizon+1, freq=df.index.inferred_freq))
        if i % shift == 0
    ]
...
def predict(df, target, features, horizon, y_lags, model, retrain=True):
    ...
    for i in range(horizon):
        X = np.array(forecast[len(forecast)-y_lags:] + list(val_df.iloc[i])).reshape(1, -1)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            y_pred = model.predict(X)[0]
        forecast.append(y_pred)
